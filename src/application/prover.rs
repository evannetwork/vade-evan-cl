...
mod tests {
    ...
    fn encodes_values_correctly() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(encoded.get("integer").unwrap().encoded, "87121".to_owned());
        assert_eq!(
        ...
        assert_eq!(
            ...
            "52530672535577884712458350945238153986666188697374769927409463847120313432331"
                .to_owned()
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "82205459161612687361280696578706529610747648852743065596896330207015226302763"
                .to_owned()
        );
        assert_eq!(
            encoded.get("false").unwrap().encoded,
            "114316671150208966788217069870207997298334791577910814811383388719888122312874"
                .to_owned()
        );
